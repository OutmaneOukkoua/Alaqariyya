...
exports.getProperties = async (req, res) => {
  ...
  // SQL query to select properties with both language-specific and generic fields
  let sql = `
    SELECT 
      p.property_id, 
      p.title_ar, p.description_ar, p.location_ar, 
      p.title_en, p.description_en, p.location_en, 
      p.price, p.bedrooms, p.bathrooms, p.salon, p.kitchen, p.area, 
  ...
  // Order the properties randomly and apply pagination
  ...
  db.query(sql, params, (err, result) => {
    ...
    db.query(countSql, countParams, (countErr, countResult) => {
      ...
      // Send back the properties along with pagination details
      res.send({
        properties: result,
